...
### `app/lib/moex_client.rb` и `app/lib/dohod_client.rb`
Два HTTP-клиента для двух источников данных. `MoexClient` — основной, для всех котировок и составов индексов. `DohodClient` — отдельный, скрейпит HTML-страницы `dohod.ru` ради прогноза дивидендов (MOEX этих данных не отдаёт).
...
- **Per-ticker scrape с rescue в `UpdateDividendForecasts`**: dohod.ru — единственный источник прогноза дивидендов, нет batch-эндпоинта. По 1 HTTP-запросу на тикер, пауза 1с между запросами. Сетевая ошибка/404 для одного тикера логируется и не прерывает обработку остальных; прежнее значение сохраняется. Пустая ячейка / прочерк / отсутствие блока на странице — это валидный сигнал "прогноза нет", записываем `NULL`.
